...
export default function AddToCartButton({ itemId, isOutOfStock }: Props) {
    ...
    useEffect(() => {
        if (!isAuthenticated) { setState("idle"); return; }
        (async () => {
            try {
                const result = await getUserCartAction();
                if (result.success && result.cart?.items) {
                    const inCart = (result.cart.items as any[]).some((item: any) => {
    ...
    // ── Full-width single-button states ──────────────────────────────
    // These render a single button that spans 100% of the parent container.
    // The parent in shop/[slug]/page.tsx is a flex-1 div, so these fill all space.

    if (isOutOfStock) return (
        <button disabled type="button" className="atc-full atc-disabled">Out of Stock</button>
    );

    if (state === "checking") return (
        <button disabled type="button" className="atc-full atc-loading">
            <span className="atc-spinner" />
        </button>
    );

    if (state === "in-cart") return (
        <button onClick={() => router.push("/cart")} type="button" className="atc-full atc-in-cart">
            <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                <path d="M2 7.5l4 4 7-7" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            Added — View Cart
            <style jsx>{`
                .atc-full { width: 100%; display: flex; align-items: center; justify-content: center; gap: 7px; height: 46px; padding: 0 20px; border-radius: 100px; border: none; font-size: 0.875rem; font-weight: 500; letter-spacing: 0.02em; cursor: pointer; white-space: nowrap; font-family: inherit; transition: all 0.2s ease; }
                .atc-in-cart { background: #EAF3EC; color: #2D6A40; border: 1.5px solid #B8D4BB; }
                .atc-in-cart:hover { background: #D4EBDA; }
            `}</style>
        </button>
    );

    // ── Qty stepper + Add to Cart button row ─────────────────────────
    return (
        <div className="atc-row">
            <div className="qty-selector">
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} disabled={quantity <= 1 || state === "loading"} className="qty-btn" type="button" aria-label="Decrease">
                ...
                <button onClick={() => setQuantity(q => q + 1)} disabled={state === "loading"} className="qty-btn" type="button" aria-label="Increase">
            ...
            <style jsx>{`
                ...
                .qty-selector {
                    ...
                    border: 1.5px solid #E8DDD0; border-radius: 100px;
                    background: #FAF7F2; overflow: hidden; height: 46px; flex-shrink: 0;
                }
                .qty-btn {
                    width: 38px; height: 46px; border: none; background: transparent;
                    display: flex; align-items: center; justify-content: center;
                    cursor: pointer; color: #6B3F2A; transition: background 0.15s;
                }
                .qty-btn:hover:not(:disabled) { background: #EDD5C0; }
                .qty-btn:disabled { opacity: 0.3; cursor: not-allowed; }
                .qty-val {
                    min-width: 32px; text-align: center; font-size: 0.9rem;
                    font-weight: 500; color: #6B3F2A;
                    border-left: 1.5px solid #E8DDD0; border-right: 1.5px solid #E8DDD0;
                    line-height: 46px;
                }
                .atc-btn {
                    flex: 1; display: flex; align-items: center; justify-content: center;
                    gap: 7px; height: 46px; padding: 0 20px; border-radius: 100px;
