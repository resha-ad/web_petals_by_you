...
import { adminGetAllDeliveriesAction } from "@/lib/actions/order-action";
...
export default function AdminDeliveriesPage() {
    ...
    const load = async () => {
        setLoading(true);
        const res = await adminGetAllDeliveriesAction(page, 15, statusFilter || undefined);
        if (res.success) {
        ...
        <div style={{ padding: "24px", maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div style={{ marginBottom: 24 }}>
                ...
                {!loading && (
                    <p style={{ color: "#9A7A7A", fontSize: "0.82rem", margin: 0 }}>
                        {pagination.total} delivery record{pagination.total !== 1 ? "s" : ""} total
                    </p>
                )}
            </div>
            ...
            {loading ? (
                ...
                <div style={{ padding: 60, textAlign: "center", background: "white", borderRadius: 14, border: "1px solid #F3E6E6", color: "#9A7A7A" }}>
                    No deliveries found{statusFilter ? ` with status "${STATUS_CONFIG[statusFilter]?.label}"` : ""}.
                </div>
            ) : (
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
                        <thead>
                            <tr style={{ borderBottom: "1.5px solid #F3E6E6", background: "#FBF6F4" }}>
                                {["Delivery ID", "Order", "Recipient", "Address", "Status", "Est. Delivery", ""].map(h => (
                        ...
                        <tbody>
                            {deliveries.map((d: any, idx: number) => {
                                ...
                                return (
                                    <tr key={d._id} style={{ borderBottom: idx < deliveries.length - 1 ? "1px solid #F9F0EE" : "none" }}>
                                        ...
                                        <td style={{ padding: "12px 16px" }}>
                                            {orderId ? (
                                                <Link href={`/admin/orders/${orderId}`} style={{ color: "#6B4E4E", textDecoration: "none", fontSize: "0.75rem", fontFamily: "monospace" }}>
                                                    #{String(orderId).slice(-8).toUpperCase()}
                                                </Link>
                                            ) : "—"}
                                        </td>
                                        <td style={{ padding: "12px 16px" }}>
                                            <p style={{ margin: 0, fontWeight: 500, color: "#6B4E4E" }}>{d.recipientName}</p>
                                            <p style={{ margin: "1px 0 0", fontSize: "0.72rem", color: "#9A7A7A" }}>{d.recipientPhone}</p>
                                        </td>
                                        <td style={{ padding: "12px 16px", color: "#7A6060", maxWidth: 200 }}>
                                            <p style={{ margin: 0 }}>{d.address?.street}</p>
                                            <p style={{ margin: "1px 0 0", fontSize: "0.72rem", color: "#9A7A7A" }}>
                                        ...
                                        <td style={{ padding: "12px 16px", color: "#9A7A7A", whiteSpace: "nowrap" }}>
                                            {d.estimatedDelivery
                                                ? new Date(d.estimatedDelivery).toLocaleDateString("en-NP", { day: "numeric", month: "short" })
                                                : "—"}
                                        </td>
                                        <td style={{ padding: "12px 16px" }}>
                                            <Link
                                                href={`/admin/deliveries/${d._id}`}
                                                style={{
            ...
            {pagination.totalPages > 1 && (
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 16, marginTop: 24 }}>
                    ...
                    <span style={{ fontSize: "0.82rem", color: "#9A7A7A" }}>Page {page} of {pagination.totalPages}</span>
                    <button
                        onClick={() => setPage(p => p + 1)}
